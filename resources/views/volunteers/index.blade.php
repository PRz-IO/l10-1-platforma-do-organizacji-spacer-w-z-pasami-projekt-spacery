                ...
                @foreach($volunteers as $volunteer)
                    <li style="margin-bottom: 15px; padding: 15px; border: 1px solid #ddd; border-radius: 5px; list-style: none; background: #fff;">
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <div>
                                <strong style="font-size: 18px;">{{ $volunteer->account->Name }} {{ $volunteer->account->Last_Name }}</strong>
                            ...
                            <div style="display: flex; gap: 8px;">
                                ...
                                @if($volunteer->account->Acc_State == 'Pending')
                                    <form action="{{ route('worker.volunteers.approve', $volunteer->Id) }}" method="POST" style="display:inline;">
                                        @csrf @method('PATCH')
                                ...
                                @if($volunteer->account->Acc_State == 'Active')
                                    <form action="{{ route('worker.volunteers.block', $volunteer->Id) }}" method="POST" style="display:inline;">
                                        @csrf @method('PATCH')
                                ...
                                <form action="{{ route('worker.volunteers.destroy', $volunteer->Id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Czy na pewno chcesz bezpowrotnie usunąć tego wolontariusza oraz jego konto z bazy danych?')">
                                    @csrf @method('DELETE')
